...
namespace dust
{
    ...
    // Item-type provides methods getHash(Key), getKey() and keyEqual(Key),
    ...
    struct Table
    {
        ...
        Table(unsigned reserveCapacity = 0) { reserve(reserveCapacity); }
    ...
    private:
        ...
        Slot & internalFind(const Key & k, bool doInsert)
        {
            ...
            for(int j = 0; j < slots.size(); ++j)
            {
                ...
                || (s.hash>>2 == hash>>2 && s.item.keyEqual(k)))
                {
                    if(doInsert)
                    {
                ...
                if(slotRemoved == (s.hash & 0x3))
                {
                    while(++j < slots.size())
                    {
                        ...
                        if(ss.hash>>2 == hash>>2 && ss.item.keyEqual(k))
                        {
                            if(doInsert)
                            {
                                // mark the slot as in use and set hash
